...
{
    MyMessage_Base *message = new MyMessage_Base("message");

    std::string vectorAsString = senderLogic(message);
    message->setM_Type(2);
    message->setM_Payload(vectorAsString.c_str());
...
void Sender::handleMessage(cMessage *msg)
{
    ...
    if(mmsg->getM_Type() == 1) {
        EV << "Receiver got error in message." << endl;
    }
    else if(mmsg->getM_Type() == 0) {
        EV << "Receiver got message successfully." << endl;
    }

    std::string vectorAsString = senderLogic(mmsg);

    mmsg->setM_Payload(vectorAsString.c_str());
    mmsg->setM_Type(2);
    send(mmsg, "out");
}
...
std::string Sender::senderLogic(MyMessage_Base* message)
{
    ...
    std::bitset<8> length (userInput.length() + 2);

    message->setM_Header(length.to_ulong());
    ...
    std::bitset<8> result(length);
    for (size_t i = 0; i < v.size(); ++i) {
        result ^= v[i];
    }

    message->setM_Trailer(result.to_ulong());
